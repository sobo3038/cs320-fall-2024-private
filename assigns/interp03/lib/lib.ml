  ...
  let type_of (env : stc_env) (e : expr) : ty_scheme option =
    ...
    try
      ...
      | Some (Forall(vars, unified_ty)) ->
         ...
         let final_vars = List.filter (fun v -> not (List.mem v env_fv)) vars in
         Some (Forall(final_vars, unified_ty))
      | None -> None
    with _ -> None
  
  
  let eval_expr env expr : value =
    let rec go env e =
      match e with
      | Unit -> VUnit
      | True -> VBool true
      ...
      | App (e1,e2) ->
        (match go env e1 with
         | VClos {env=v_env; name=v_name; arg=v_arg; body=v_body} ->
           ...
           go (Env.add v_arg (go env e2) v_env) v_body
         | _ -> failwith "impossible")
      | Bop (Add,a,b) ->
        (match go env a,go env b with VInt m,VInt n->VInt(m+n)|_->failwith"impossible")
      | Bop (AddF,a,b)->
        (match go env a,go env b with VFloat m,VFloat n->VFloat(m+.n)|_->failwith"impossible")
      | Bop (Sub,a,b)->
        (match go env a,go env b with VInt m,VInt n->VInt(m-n)|_->failwith"impossible")
      | Bop (SubF,a,b)->
        (match go env a,go env b with VFloat m,VFloat n->VFloat(m-.n)|_->failwith"impossible")
      ...
      | Bop (Div,a,b)->
        (match go env a with
         | VInt m -> (match go env b with VInt 0->raise DivByZero|VInt n->VInt(m/n)|_->failwith"")
         | _->failwith"")
      | Bop (DivF,a,b)->
        (match go env a with
         | VFloat m->(match go env b with VFloat n->VFloat(m/.n)|_->failwith"")
         | _->failwith"")
      | Bop (Mod,a,b)->
        (match go env a,go env b with
      ...
      | Bop (And,a,b)->
        (match go env a with VBool false->VBool false|VBool true->go env b|_->failwith"Logical 'and' requires boolean operands")
      | Bop (Or,a,b)->
        (match go env a with VBool true->VBool true|VBool false->go env b|_->failwith"Logical 'or' requires boolean operands")
      | ESome e ->
        VSome (go env e)
      | OptMatch{matched;some_name;some_case;none_case} ->
      ...
      | If(c,t,f)->
        (match go env c with VBool true->go env t|VBool false->go env f|_->failwith"Condition in if-expression must be a boolean")
      | Bop (Comma,a,b)->
        let va=go env a in
        let vb=go env b in
        VPair(va,vb)
      ...
      | ListMatch{matched;hd_name;tl_name;cons_case;nil_case} ->
        (match go env matched with
         | VList(h::t)->
           go (Env.add tl_name (VList t)(Env.add hd_name h env)) cons_case
         | VList []->go env nil_case
         | _->failwith"Expected a list")
      | PairMatch{matched;fst_name;snd_name;case} ->
        (match go env matched with
         | VPair(x,y)->
           go (Env.add snd_name y (Env.add fst_name x env)) case
      ...
      | Assert e1->
        (match go env e1 with VBool true->VUnit|VBool false->raise AssertFail|_->raise AssertFail)
      | Let{is_rec=false;name;value;body} ->
      ...
      | Let{is_rec=true;name=f;value=v;body} ->
        ...
        (match c with
         | VClos{name=None;arg;body=cb;env=ce} ->
           go (Env.add f (VClos{name=Some f;arg;body=cb;env=ce}) env) body
