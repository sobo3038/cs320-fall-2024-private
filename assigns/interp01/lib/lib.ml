...
let rec subst (v: value) (x: string) (e: expr) : expr =
  let rec replace_var new_name old_name expr =
    ...
    | Fun (param, body) ->  
        if param = old_name then Fun (param, body)
        else Fun (param, replace_var new_name old_name body)
    | Let (y, e1, e2) ->  
        Let (y, replace_var new_name old_name e1, replace_var new_name old_name e2)
    | If (e1, e2, e3) ->  
        If (replace_var new_name old_name e1, replace_var new_name old_name e2, replace_var new_name old_name e3)
    | Bop (op, e1, e2) ->  
        Bop (op, replace_var new_name old_name e1, replace_var new_name old_name e2)
    | _ -> expr  
  in
  match e with
  | Num n -> Num n  
